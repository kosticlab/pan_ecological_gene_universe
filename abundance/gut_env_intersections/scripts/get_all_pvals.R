...
get_abundance_per_ecosystem_no_outliers_v2 = function(database,mydf,sample_cat_db,samples_of_interest,gene_mapping,output_folder) {
  samples_not_interest = sample_cat_db$sample_type[!sample_cat_db$sample_type%in%samples_of_interest]
  myTab1 = fread(cmd= paste('zcat',mydf[1,1]),sep="\t",header=F)
  congenes_raw_mat = apply(mydf, 1, function(myrow) {
  myTab2 = fread(cmd= paste('zcat',myrow[1],'| cut -f3'),sep="\t",header=F)
  return(myTab2)
})
...
# remove row that has a star
congenes_raw_mat = congenes_raw_mat[rownames(congenes_raw_mat) != "*",]
...
# order mapping file
gene_prev_mapping_ordered = gene_prev_mapping[match(sig_gene_list_DF$geneName,rownames(congenes_raw_mat_gene_filt)),]
sig_gene_list_DF = cbind(sig_gene_list_DF,prev_categories=gene_prev_mapping_ordered$V2)
write.table(sig_gene_list_DF,file=paste(output_folder,"/",database,'_pvals.txt',sep=""),quote=F,sep="\t",col.names=T,row.names=F)
}

pvalList = get_abundance_per_ecosystem_no_outliers_v2(database=database,mydf=file_df,sample_cat_db=sample_categories,samples_of_interest=samples_of_interest,gene_mapping=gene_mapping,output_folder=output_folder)
